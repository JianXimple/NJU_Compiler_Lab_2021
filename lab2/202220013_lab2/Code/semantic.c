...
int cmp_array(Type* t1, Type* t2)
{
    ...
    while (t1 && t2) {
        if (t1 == t2)
            ...
            return 0;
        } else if (t1->u.array.elem->kind == STRUCTURE || t1->u.array.elem->kind == STRUCTURE) {
            res = cmp_structure(t1->u.array.elem->u.structure, t2->u.array.elem->u.structure);
            if (!res)
                return 0;
            else
                return 1;
...
int cmp_function(FieldList* arg1, FieldList* arg2)
{
    ...
    while (arg1 && arg2) {
        if (arg1 == arg2)
            ...
            return 0;
        } else if (arg1->type->kind == STRUCTURE) {
            res = cmp_structure(arg1->type->u.structure, arg2->type->u.structure);
            if (!res)
                return 0;
        } else if (arg1->type->kind == ARRAY) {
            res = cmp_array(arg1->type, arg2->type);
            if (!res)
                return 0;
        } else
            res = 1;
...
void ExtDef(Node* node)
{
    ...
    case 3: // Specifier FunDec CompSt
        ...
        if (node->child->syn->type->kind == STRUCTURE) {
            Type* type = (Type*)malloc(sizeof(Type));
            type->kind = STRUCTURE;
            type->u.structure = node->child->syn->type->u.structure;
...
void VarDec(Node* node)
{
    ...
    case 1: // ID
        ...
        if (!res) {
            ...
            case STRUCTURE: {
                ...
                Type* type = (Type*)malloc(sizeof(Type));
                type->kind = STRUCTURE;
                type->u.structure = node->inh->type->u.structure;
                attr->name = node->child->val.type_str;
                attr->type = type;
                attr->tail = NULL;
                item->type = type;
    ...
    case 2: // VarDec LB INT RB
    {
        ...
        if (node->inh->type->kind == STRUCTURE) {
            ...
            p->kind = STRUCTURE;
            p->u.structure = node->inh->type->u.structure;
            type->u.array.elem = p;
        } else {
    ...
    case 3: // RETURN Exp SEMI
        ...
        if (childAt(node, 1)->syn) {
            if (node->inh->type->kind != childAt(node, 1)->syn->type->kind) {
                if (node->inh->type->kind == STRUCTURE && childAt(node, 1)->syn->type->kind == STRUCTURE) {
                    int res = cmp_structure(node->inh->type->u.structure, childAt(node, 1)->syn->type->u.structure);
                    if (res)
                        break;
                }
                log_error(8, childAt(node, 1)->line, "Type mismatched for return");
            }
        }
        break;
    case 4: // IF LP Exp RP Stmt
        ...
        if (childAt(node, 2)->syn) {
            if (childAt(node, 2)->syn->type->kind != INT) {
                log_error(7, childAt(node, 2)->line, "Type mismatched for operands");
            }
        }
        Stmt(childAt(node, 4));
        break;
...
void Exp(Node* node)
{
    ...
    case 1: // Exp ASSIGNOP Exp
    {
        ...
        if (id < 14 || id > 16) {
            log_error(6, node->child->line, "The left-hand side of an assignment must be a variable");
            Exp(childAt(node, 2));
            return;
        } else {
            Exp(node->child);
        ...
        if (node->child->syn && childAt(node, 2)->syn) {
            if (node->child->syn->type->kind != childAt(node, 2)->syn->type->kind) {
                log_error(5, node->child->line, "Type mismatched for assignment");
            } else if (node->child->syn->type->kind == STRUCTURE) {
                FieldList* p = node->child->syn->type->u.structure;
                FieldList* q = childAt(node, 2)->syn->type->u.structure;
                int res = cmp_structure(p, q);
                if (!res) {
                    log_error(5, node->child->line, "Type mismatched for assignment");
                } else {
                    node->syn = node->child->syn;
    ...
    case 2: // Exp AND Exp
        ...
        if (node->child->syn && childAt(node, 2)->syn) {
            if (node->child->syn->type->kind != INT || childAt(node, 2)->syn->type->kind != INT) {
                log_error(7, node->child->line, "Type mismatched for operands");
            } else {
                Type* type = (Type*)malloc(sizeof(Type));
                type->kind = INT;
                FieldList* attr = (FieldList*)malloc(sizeof(FieldList));
    ...
    case 10: // MINUS Exp
        ...
        if (childAt(node, 1)->syn) {
            if (childAt(node, 1)->syn->type->kind == INT || childAt(node, 1)->syn->type->kind == FLOAT) {
                ...
                log_error(7, node->line, "Type mismatched for operands");
            }
        }
        break;
    case 11: // NOT Exp
    ...
    case 12: // ID LP Args RP
        ...
        if (res) {
            if (res->type->kind != FUNC) {
                ...
                log_error(11, node->line, msg);
            } else {
                int tmp = cmp_function(res->type->u.function.args, childAt(node, 2)->syn);
                if (tmp) {
                    FieldList* attr = (FieldList*)malloc(sizeof(FieldList));
                    attr->tail = NULL;
                    attr->name = NULL;
                    ...
                    log_error(9, childAt(node, 2)->line, msg);
                }
            }
        } else {
            sprintf(msg, "Undefined function \"%s\"", node->child->val.type_str);
            log_error(2, node->line, msg);
            return;
        }
        break;
    case 13: // ID LP RP
    ...
    case 14: // Exp LB Exp RB
    {
        ...
        if (attr->type->kind != ARRAY) {
            ...
            log_error(10, node->child->line, msg);
        } else {
        ...
        if (attr->type->kind != INT) {
            ...
            log_error(12, childAt(node, 2)->line, msg);
        }
    } break;
    case 15: // Exp DOT ID
    {
        ...
        if (!attr)
            return;
        if (attr->type->kind != STRUCTURE) {
            log_error(13, node->child->line, "Illegal use of \".\"");
        } else {
            int flag = 0;
            for (FieldList* p = attr->type->u.structure; p; p = p->tail) {
            ...
            if (!flag) {
                ...
                log_error(14, node->child->line, msg);
            }
        }
    } break;
    case 16: // ID
...
void Dec(Node* node)
{
    ...
    case 2: // VarDec ASSIGNOP Exp
        if (node->instruct) {
            ...
            log_error(15, node->child->line, msg);
            return;
        }
        Exp(childAt(node, 2));
        if (node->child->syn && childAt(node, 2)->syn) {
