...
int calOffset(Operand* op, int fpoffset)
{
...
int allocVar(InterCodes* begin)
{
    ...
    while (p && p->code.kind != FUNCTION) {
        if (!p->isDelete)
            switch (p->code.kind) {
            case ASSIGN: {
                fpoffset += calOffset(p->code.u.assign.left, fpoffset);
                fpoffset += calOffset(p->code.u.assign.right, fpoffset);
            } break;
            case ADD:
            case SUB:
            case MUL:
            case DIV: {
                fpoffset += calOffset(p->code.u.binop.res, fpoffset);
                fpoffset += calOffset(p->code.u.binop.op1, fpoffset);
                fpoffset += calOffset(p->code.u.binop.op2, fpoffset);
            } break;
            case IF: {
                fpoffset += calOffset(p->code.u.cond.op1, fpoffset);
                fpoffset += calOffset(p->code.u.cond.op2, fpoffset);
            } break;
            case DEC:
                ...
                break;
            case CALL: {
                fpoffset += calOffset(p->code.u.sinop.res, fpoffset);
            } break;
            case RETURN:
            case ARG:
            case PARAM:
            case READ:
            case WRITE:
                fpoffset += calOffset(p->code.u.single.op, fpoffset);
                break;
            }
        p = p->next;
    }
    ...
    while (p) {
        if (!p->isDelete)
            ...
            case ASSIGN: {
                ...
                switch (p->code.type) {
                case NORMAL: {
                    reg_index = varOffset[l->u.var_no + loffset];
                    if (r->kind == CONSTANT) {
                        fprintf(f, "  li $t0, %lld\n", r->u.value);
                    } else {
                        int roffset = 0;
            ...
            case ADD: {
                ...
                reg_index = varOffset[res->u.var_no + resoffset];
